...
#
# asan_options.c provides a STRONG __asan_default_options / __lsan_default_options that disables
# LeakSanitizer (detect_leaks=0).  LSan ptrace-attaches to its own threads at process exit to scan
# for leaks, but Mayhem already runs the target under ptrace for coverage collection — a process can
# have only one ptrace tracer, so LSan aborts before any edges are recorded (has_critical_errors /
# 0-edge "Run Failed").  The strong symbol wins over the ASan runtime's weak copy regardless of link
# order, even when the whole instrumented libabc.a is pulled in.
$CC  $SAN $DEBUG_FLAGS -Wall -c "$SRC/src/demo.c"             -I"$SRC/src" -o /tmp/demo.o
$CC  $SAN $DEBUG_FLAGS -Wall -c "$SRC/mayhem/asan_options.c"               -o /tmp/asan_options.o
$CXX $SAN $DEBUG_FLAGS -o /mayhem/demo /tmp/demo.o /tmp/asan_options.o "$SRC/libabc.a" -lm -ldl -lrt -lpthread
